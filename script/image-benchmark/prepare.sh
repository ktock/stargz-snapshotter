...
mkdir -p "${CONTAINERD_CONFIG_DIR}" "${REMOTE_SNAPSHOTTER_CONFIG_DIR}" "${CNI_CONFIG_DIR}" && \
    cp "${REPO}/script/image-benchmark/config.containerd.toml" "${CONTAINERD_CONFIG_DIR}config.toml" && \
    cp "${REPO}/script/image-benchmark/config.stargz.toml" "${REMOTE_SNAPSHOTTER_CONFIG_DIR}config.toml" && \
    cp "${REPO}/script/image-benchmark/config.cni.conflist" "${CNI_CONFIG_DIR}optimizer.conflist"

if [ "${METADATA_STORE:-}" == "db" ] ; then
    sed -i 's/metadata_store = "memory"/metadata_store = "db"/g' "${REMOTE_SNAPSHOTTER_CONFIG_DIR}config.toml"
fi
cat "${REMOTE_SNAPSHOTTER_CONFIG_DIR}config.toml"
