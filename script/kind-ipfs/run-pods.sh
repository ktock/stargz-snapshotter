...
wait_for_running "${KIND_KUBECONFIG}" "app=ipfs-cluster"
KUBECONFIG="${KIND_KUBECONFIG}" kubectl get all

KIND_NODENAME=$(kind get nodes --name "${KIND_CLUSTER_NAME}" | grep -v control-plane | sed -n 1p)
docker exec "${KIND_NODENAME}" /bin/bash -c "mkdir -p /tmp/ipfsapi ; echo -n /ip4/127.0.0.1/tcp/9095 >  /tmp/ipfsapi/api"
