...
# TESTING_MANIFESTS=script/nerdctl-ipfs-registry-kubernetes
# IPFS_API_ADDR=/ip4/127.0.0.1/tcp/5001
TESTING_MANIFESTS=script/nerdctl-ipfs-registry-kubernetes-cluster
IPFS_API_ADDR=/ip4/127.0.0.1/tcp/9095
...
"${REPO}"/"${TESTING_MANIFESTS}"/bootstrap.yaml.sh | KUBECONFIG="${KIND_KUBECONFIG}" kubectl apply -f -
KUBECONFIG="${KIND_KUBECONFIG}" kubectl apply -f "${REPO}"/"${TESTING_MANIFESTS}"/nerdctl-ipfs-registry.yaml
...
docker exec "${KIND_NODENAME}" /bin/bash -c "mkdir -p /tmp/ipfsapi ; echo -n ${IPFS_API_ADDR} >  /tmp/ipfsapi/api"
