...
LOG_FILE=$(mktemp)
MIRROR_TMP=$(mktemp -d)
C_STORAGE_CONFIG=$(mktemp)
function cleanup {
    ORG_EXIT_CODE="${1}"
    docker compose --progress=plain -f "${DOCKER_COMPOSE_YAML}" down -v || true
...
EOF
docker compose --progress=plain -f "${DOCKER_COMPOSE_YAML}" up -d --force-recreate
...
echo "ghcr.io/stargz-containers/ubuntu:20.04-esgz" > "${MIRROR_TMP}/list"
...
cat "${MIRROR_TMP}/list" | sed -E 's/^([^/]*).*/\1/g' | sort | uniq | while read DOMAIN ; do
...
docker cp "${CONTEXT}/test-restart.sh" "${TEST_NODE_NAME}":/
...
docker exec "${TEST_NODE_NAME}" /test-restart.sh
