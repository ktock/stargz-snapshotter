...
function check_remote_snapshots_bk {
    local TARGET_LOG_FILE="${1}"
    local REMOTE=0
    local LOCAL=0

    REMOTE=$(cat "${TARGET_LOG_FILE}" | { grep "remote-snapshot-prepared=true" || true; } | wc -l)
    LOCAL=$(cat "${TARGET_LOG_FILE}" | { grep "remote-snapshot-prepared=false" || true; } | wc -l)
    if [[ ${LOCAL} -gt 0 ]] ; then
...
for SAMPLE_NO in $(seq ${NUM_OF_SAMPLES}) ; do
    ...
    for THEWL in $(cat "${WORKLOADS_LIST}") ; do
        ...
        if [ "${MODE}" == "${ESTARGZ_NOOPT_MODE}" ] ; then
            ...
            LOG_FILE="${TMP_LOG_FILE}" measure "${IMAGE}" "${MODE}" "stargz" "esgz-noopt"
            check_remote_snapshots_bk "${TMP_LOG_FILE}"
        fi
