...
set title "build of ${IMGNAME}/${MODE} (${SAMPLES}samples)"
...
mkdir -p "${RAWDATADIR}" "${PLTDATADIR}" "${IMGDATADIR}" "${CSVDATADIR}"
for IMGNAME in "${IMAGES[@]}" ; do
    echo "Processing: ${IMGNAME}"
    IMAGE=$(echo "${IMGNAME}" | sed 's|[/:]|-|g')
    CSVFILE="${CSVDATADIR}result-${IMAGE}.csv"
    echo "image,mode,percentile,time" > "${CSVFILE}"
    for MODE in "${MODES[@]}"; do
        DATAFILE="${RAWDATADIR}${IMAGE}-${MODE}.dat"
        PLTFILE="${PLTDATADIR}result-${IMAGE}-${MODE}.plt"
        template "${IMGDATADIR}result-${IMAGE}-${MODE}.png" \
                 "${IMGNAME}" "${MODE}" "${MINSAMPLES}" > "${PLTFILE}"
        for PCTL in $(seq 0 "${GRANULARITY}" 100) ; do
            TIME=$(PERCENTILE=${PCTL} percentile "${JSON}" "${MINSAMPLES}" "${IMGNAME}" "${MODE}" "elapsed")
            echo "${PCTL} ${TIME}" >> "${DATAFILE}"
            echo "${IMGNAME},${MODE},${PCTL},${TIME}" >> "${CSVFILE}"
        done
