...
`ctr-remote i convert` supports optional flag `--estargz-lossless` specified with `--estargz-external-toc`.
...
```console
# ctr-remote i pull ghcr.io/stargz-containers/ubuntu:22.04
# ctr-remote i convert --oci --estargz --estargz-external-toc --estargz-lossless ghcr.io/stargz-containers/ubuntu:22.04 registry2:5000/ubuntu:22.04-ex-ll
# ctr-remote i push --plain-http registry2:5000/ubuntu:22.04-ex-ll
# ctr-remote i push --plain-http registry2:5000/ubuntu:22.04-ex-ll-esgztoc
# crane --insecure blob registry2:5000/ubuntu:22.04-ex-ll@sha256:2dc39ba059dcd42ade30aae30147b5692777ba9ff0779a62ad93a74de02e3e1f | jq -r '.rootfs.diff_ids[]'
...
This is possible because when we create eStargz with `--estargz-external-toc`, we don't need to modify (i.e. adding TOC file) the original tar layer blob.
...
> NOTE: This feature uses [experimental v2 spec](./estargz_v2.md) of eStargz so older version of Stargz Snapshotter cannot perform lazy pulling for the images created with this flag.
