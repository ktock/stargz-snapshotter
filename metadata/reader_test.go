...
// TestRead tests Reader returns correct file metadata.
		...
		for _, prefix := range allowedPrefix {
			...
			for _, cLevel := range compressionLevels {
				...
				t.Run(tt.name, func(t *testing.T) {
					...
					if err != nil {
						t.Fatalf("failed to create tmp db file: %v", err)
					}
					defer f.Close()
					defer os.Remove(f.Name())
					db, err := bolt.Open(f.Name(), 0666, nil)
					if err != nil {
						t.Fatalf("failed to open tmp db file: %v", err)
					}
					defer db.Close()
					opts := []testutil.BuildEStargzOption{
						testutil.WithBuildTarOptions(testutil.WithPrefix(prefix)),
						testutil.WithEStargzOptions(estargz.WithCompressionLevel(cLevel)),
					}
					if tt.chunkSize > 0 {
					...
					r, err := NewReader(xid.New().String(), db, esgz)
					if err != nil {
...
func dumpNodes(t *testing.T, r *Reader, id uint32, level int) {
...
type check func(*testing.T, *Reader)
	...
	return func(t *testing.T, r *Reader) {
		if err := r.view(func(tx *bolt.Tx) error {
			...
			if err := nodes.ForEach(func(k, v []byte) error {
				...
				t.Logf("  +- [%d] name:%q", firstID, string(firstName))
			...
			if want != i {
				return fmt.Errorf("unexpected num of nodes %d; want %d", i, want)
			}
			return nil
		}); err != nil {
...
func numOfChunks(name string, num int) check {
	return func(t *testing.T, r *Reader) {
		...
		if err := r.view(func(tx *bolt.Tx) error {
			...
			t.Errorf("num of chunks: %v", err)
			return
		}
		t.Logf("[%d] %q chunks", id, name)
		for _, c := range chunks {
			t.Logf("- %+v", c)
		}
		if len(chunks) != num {
			t.Errorf("unexpected num of chunk of %q : %d want %d", name, len(chunks), num)
		}
	}
}
...
func sameNodes(n string, nodes ...string) check {
	return func(t *testing.T, r *Reader) {
		...
		for _, en := range nodes {
			...
			if eid != id {
				t.Errorf("unexpected ID of %q: %d want %d", en, id, eid)
			}
		}
	}
}
...
func hasModTime(name string, modTime time.Time) check {
	return func(t *testing.T, r *Reader) {
		...
		// attrModTime, _ := time.Parse(time.RFC3339, attr.ModTime3339)
