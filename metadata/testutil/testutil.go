...
func TestReader(t *testing.T, factory ReaderFactory) {
	...
	data64KB := tutil.RandomContents(64000)
	tests := []struct {
		name         string
		chunkSize    int
		minChunkSize int
...
func hasFile(name, content string, size int64) check {
	return func(t *testing.T, r TestableReader) {
		...
		if attr.Size != size {
			t.Errorf("unexpected size of file %q : %d (%q) want %d (%q)", name, attr.Size, tutil.ViewContents(data), size, tutil.ViewContents([]byte(content)))
			return
		}
		if string(data) != content {
			t.Errorf("unexpected content of %q: %q want %q", name, tutil.ViewContents(data), tutil.ViewContents([]byte(content)))
			return
		}
	}
}
...
func hasFileContentsWithPreRead(name string, off int64, contents string, extra ...chunkInfo) check {
	return func(t *testing.T, r TestableReader) {
		...
		if string(buf) != contents {
			t.Errorf("unexpected content of %q: %q want %q", name, tutil.ViewContents(buf), tutil.ViewContents([]byte(contents)))
			return
		}
		if len(extraMap) != 0 {
			var exNames []string
