...
func (gc *GzipCompressor) WriteTOCAndFooter(w io.Writer, off int64, toc *estargz.JTOC, diffHash hash.Hash) (digest.Digest, error) {
	...
	exTOCDigest := digest.Canonical.Digester()
	gz, _ := gzip.NewWriterLevel(io.MultiWriter(buf, exTOCDigest.Hash()), gc.compressionLevel)
	...
	gc.buf = buf
	if _, err := w.Write(gzipFooterBytes(exTOCDigest.Digest())); err != nil {
		return "", err
	}
	return digest.FromBytes(tocJSON), nil
}

const FooterSize = 104

// gzipFooterBytes returns the 104 bytes footer.
func gzipFooterBytes(tocDigest digest.Digest) []byte {
	...
	subfield := tocDigest.Encoded() + "EXTERNALSHA"                   // 64(sha256) + 11(EXTERNALSHA) = 75
...
func (gz *GzipDecompressor) ParseFooter(p []byte) (blobPayloadSize, tocOffset, tocSize int64, err error) {
	...
	if slen := binary.LittleEndian.Uint16(subfieldlen); slen != uint16(64+len("EXTERNALSHA")) {
		return 0, 0, 0, fmt.Errorf("invalid length of subfield %d; want %d", slen, 16+len("STARGZ"))
	}
	if string(subfield[64:]) != "EXTERNALSHA" {
		return 0, 0, 0, fmt.Errorf("STARGZ magic string must be included in the footer subfield")
	}
	if _, err := digest.Parse("sha256:" + string(subfield[:64])); err != nil {
		return 0, 0, 0, err
	}
