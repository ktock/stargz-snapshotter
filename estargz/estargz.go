...
func Open(sr *io.SectionReader, opt ...OpenOption) (*Reader, error) {
	...
	if r.toc.Version > supportedTOCVersion {
		// TODO: optionally allow continuing instead of returning an error.
		return nil, fmt.Errorf("unsupported TOC version %d is newer than %d", r.toc.Version, supportedTOCVersion)
	}
...
type Writer struct {
	...
	// stream is started.
	// Note that old version of decompressor can't parse the result
	// blob.
	MinChunkSize int
...
// you shoud decompress it and remreove TOC JSON in advance.
		...
		if h.Typeflag == tar.TypeReg && ent.Size > 0 {
			...
			for written < totalSize {
				...
				if w.needsOpenGz(ent) || w.cw.n-prevOffset >= int64(w.MinChunkSize) {
					...
					w.toc.Version = 2 // Upgrade TOC version; "InnerOffset" field requires Version >= 2
