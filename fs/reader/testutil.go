...
func hasFileContentsOffset(name string, off int64, contents string, fromCache bool) check {
	return func(t *testing.T, r *reader, cr *calledReaderAt) {
		...
		if n != len(contents) {
			t.Fatalf("failed to read contents %q (off:%d, want:%q) got %q", name, off, contents, string(buf))
		}
		if string(buf) != contents {
			t.Fatalf("unexpected content of %q: %q want %q", name, testutil.ViewContents(buf), testutil.ViewContents([]byte(contents)))
		}
	...
	return func(t *testing.T, r *reader, cr *calledReaderAt) {
		...
		for _, e := range extra {
			...
			if string(data) != e.data {
				t.Fatalf("unexpected contents of cache %q (%+v): %q; wanted %q", cacheID, e, testutil.ViewContents(data), testutil.ViewContents([]byte(e.data)))
			}
		}
	}
}
