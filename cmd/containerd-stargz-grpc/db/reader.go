...
func (r *reader) init(decompressedR io.Reader, rOpts metadata.Options) (retErr error) {
	...
	// Ensure the TOC version is supported.
	// NOTE: This assumes "version" field comes before "entries" field. If not, initialization fails.
	// TODO: support TOC JSON where "version" field comes after "entries".
	var version int
	for {
		t, err := dec.Token()
		if err != nil {
	...
	if version == 0 {
		return fmt.Errorf("unknown TOC version")
	} else if version > supportedTOCVersion {
		return fmt.Errorf("unsupported TOC version %d > %d", version, supportedTOCVersion)
	}
	...
	r.initG.Go(func() error {
		defer closeFunc()
		if err := r.initNodes(dec); err != nil {
			return err
		}
...
func (r *reader) initNodes(dec *json.Decoder) error {
...
func getVersion(dec *json.Decoder, t json.Token) (int, bool, error) {
	if ele, ok := t.(string); ok {
		if ele == "version" {
			vt, err := dec.Token()
			if err != nil {
				return 0, false, fmt.Errorf("failed to get JSON version token: %w", err)
			}
			n, ok := vt.(float64)
			if !ok {
				return 0, false, fmt.Errorf("failed to get JSON number token (%T: %v)", vt, vt)
			}
