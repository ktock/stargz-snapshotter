...
`,
	Flags: []cli.Flag{
		...
		cli.IntFlag{
			...
			Usage: "The minimal number of bytes of data must be written in one gzip stream. The result blob is still gzip-compatible but old eStargz reader cannot do random access.",
			Value: 0,
		},
		cli.BoolFlag{
			Name:  "estargz-external-toc",
			Usage: "convert to esgz without embedding TOC into the blob",
		},
		cli.BoolFlag{
	...
	Action: func(context *cli.Context) error {
		...
		if context.Bool("estargz") {
			...
			if context.Bool("estargz-external-toc") {
				if !context.Bool("estargz-lossless") {
					...
					if context.String("estargz-record-in") != "" {
						return fmt.Errorf("option --estargz-lossless conflicts with --estargz-record-in")
					}
				...
				if context.Bool("estargz-lossless") {
					return fmt.Errorf("option --estargz-lossless must be used with --estargz-external-toc")
				}
